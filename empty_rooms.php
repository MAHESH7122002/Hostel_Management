...
<?php
   if (isset($_POST['search'])) {
   	   ...
   	   $query_search = "SELECT Room_No,count(*) as num_rooms FROM Room WHERE Room_No like '$search_box%' and Hostel_id = '$hostel_id' and Allocated = '0' group by Room_No";
    ...
    <?php
   	   if(mysqli_num_rows($result_search)==0){
      		...
      		echo "<tr><td>{$row_search['Room_No']}</td><td>{$row_search['num_rooms']}</td></tr>\n";
   	   }
   }
   ?>
   </tbody>
...
<?php
   $hostel_id = $_SESSION['hostel_id'];
   $query1 = "SELECT Room_No,count(*) as num_rooms FROM Room where Hostel_id = '$hostel_id' and Allocated = '0' group by Room_No ";
      ...
      if(mysqli_num_rows($result1)==0){
         echo '<tr><td colspan="4">No Rows Returned</td></tr>';
      	...
      	while($row1 = mysqli_fetch_assoc($result1)){
      		echo "<tr><td>{$row1['Room_No']}</td><td>{$row1['num_rooms']}</td></tr>\n";
      	}
      }
    ?>
    </tbody>
